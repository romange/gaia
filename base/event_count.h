...
class EventCount {
 public:
  ...
  bool wait(Key key, MicrosecondsInt64 now, MicrosecondsInt64 until) noexcept;
  ...
  /**
   * Wait for condition() to become true.  Will clean up appropriately if
   * condition() throws, and then rethrow. Waits at most timeout_usec (may be more due to
   * limit clock precision or thread switching)
   * and if condition is still false - returns false. Otherwise returns true.
   */
  template <class Condition>
  bool await(Condition condition, MicrosecondsInt64 timeout_usec);
 private:
...
inline bool EventCount::notifyAll() noexcept {
  return doNotify(INT_MAX);
}

inline bool EventCount::doNotify(int n) noexcept {
  // Invalidate epoch number so that all waiting threads that are checking it agains their
  // key.epoch_ will have to evaluate their condition again.
  uint64_t prev = val_.fetch_add(kAddEpoch, std::memory_order_acq_rel);
  if (PREDICT_FALSE(prev & kWaiterMask)) {
    // FUTEX_WAKE_PRIVATE, FUTEX_WAIT_PRIVATE use private futex, not shared with other processes.
...
inline void EventCount::cancelWait() noexcept {
  ...
  uint64_t prev ATTRIBUTE_UNUSED = val_.fetch_add(kSubWaiter, std::memory_order_seq_cst);
...
inline void EventCount::wait(Key key) noexcept {
  while ((val_.load(std::memory_order_acquire) >> kEpochShift) == key.epoch_) {
    detail::futex(reinterpret_cast<int*>(&val_) + kEpochOffset,
                  FUTEX_WAIT_PRIVATE, key.epoch_, nullptr, nullptr, 0);
  }
  // memory_order_relaxed would suffice for correctness, but the faster
  // #waiters gets to 0, the less likely it is that we'll do spurious wakeups
  // (and thus system calls)
...
inline bool EventCount::wait(Key key, MicrosecondsInt64 now, MicrosecondsInt64 until) noexcept {
  ...
  while ((val_.load(std::memory_order_acquire) >> kEpochShift) == key.epoch_) {
    ...
    MicrosecondsInt64 delta = until - now;
    if (delta <= 0)
      return false;
    ts.tv_sec = delta / 1000000;
    ts.tv_nsec = (delta % 1000000)*1000;

    int res = detail::futex(reinterpret_cast<int*>(&val_) + kEpochOffset,
                  FUTEX_WAIT_PRIVATE, key.epoch_, &ts, nullptr, 0);
...
template <class Condition>
bool EventCount::await(Condition condition, MicrosecondsInt64 timeout_usec) {
  ...
  try {
    ...
    for (;;) {
      ...
      if (condition()) {
        ...
        if (!wait(key, now, now + timeout_usec))
          return false;
        now = 0;
      }
    }
