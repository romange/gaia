...
class LittleEndian {
 public:
  ...
  static void Store24(void *p, uint32 v) {
    uint8* ptr = reinterpret_cast<uint8*>(p);
    *ptr = v;
  ...
  static uint32 Load24(const void *p) {
    const uint8* ptr = reinterpret_cast<const uint8*>(p);

    return (uint32(ToHost16(UNALIGNED_LOAD16(ptr + 1))) << 8) | *ptr;
  }
  ...
  template<unsigned L> void Store64BoundedT(void* p, uint64_t v) {
    static_assert(L > 0 && L < sizeof(uint64_t), "");
    uint64_t tmp = FromHost64(v);
    memcpy(p, &tmp, L);
  }
...
template<> inline void LittleEndian::StoreT<uint32>(uint32 val, void* dest) {
  Store32(dest, val);
}

template<> inline void LittleEndian::StoreT<uint64>(uint64 val, void* dest) {
  Store64(dest, val);
}

template<> inline uint32_t LittleEndian::LoadT<uint32>(const void* src) {
  return Load32(src);
}

template<> inline uint64_t LittleEndian::LoadT<uint64>(const void* src) {
  return Load64(src);
}
