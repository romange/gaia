...
/**
  * Default c'tor does not allocate. On the first allocation it allocates at least INITIAL_SIZE bytes.
  *
...
template<size_t ELEM_SIZE, size_t ALIGNMENT> class PODArrayBase {
 ...
 protected:
  ...
  PODArrayBase(pmr::memory_resource* mr) : mr_(mr ? mr : pmr::get_default_resource()) {}

  void alloc(size_t bytes) {
    bytes = Bits::RoundUp64(bytes);
 ...
 public:
  size_t allocated_size() const { return c_end_of_storage_ - c_start_; }
  void clear() {
    c_end_ = c_start_;
  }
  ...
  bool empty() const { return c_end_ == c_start_; }
  size_t capacity() const { return (c_end_of_storage_ - c_start_) / ELEM_SIZE; }
  size_t size() const { return (c_end_ - c_start_) / ELEM_SIZE; }
  ...
  pmr::memory_resource* mr() { return mr_; }
};
...
template <typename T, size_t ALIGNMENT = 16 >
 ...
 private:
  ...
  T * t_start()           { return reinterpret_cast<T *>(c_start_); }
  T * t_end()             { return reinterpret_cast<T *>(c_end_); }
  T * t_end_of_storage()  { return reinterpret_cast<T *>(c_end_of_storage_); }

  const T * t_start() const     { return reinterpret_cast<const T *>(c_start_); }
  const T * t_end() const       { return reinterpret_cast<const T *>(c_end_); }
  const T * t_end_of_storage() const  { return reinterpret_cast<const T *>(c_end_of_storage_); }
...
public:
  ...
  PODArray(::pmr::memory_resource* mr = nullptr) : ParentClass(mr) {
  }

  PODArray(size_t n, ::pmr::memory_resource* mr) : ParentClass(mr) {
    alloc_for_num_elements(n);
    c_end_ += byte_size(n);
  }

  PODArray(size_t n, const T & x, ::pmr::memory_resource* mr) : ParentClass(mr) {
    alloc_for_num_elements(n);
    assign(n, x);
  }

  PODArray(const_iterator from_begin, const_iterator from_end,
           ::pmr::memory_resource* mr) : ParentClass(mr) {
  ...
  void push_back(const T & x) {
    if (UNLIKELY(c_end_ == c_end_of_storage_))
      reserve();
  ...
  void insert(It1 from_begin, It2 from_end) {
    ...
    if (required_capacity > capacity())
      reserve(Bits::RoundUp(required_capacity));
