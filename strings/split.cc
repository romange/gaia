...
StringPiece GenericFind(
  ...
  if (delimiter.empty() && !text.empty()) {
    ...
    return StringPiece(text.begin() + 1, 0UL);
  }
  size_t found_pos = StringPiece::npos;
  StringPiece found(text.end(), 0UL);  // By default, not found
  found_pos = find_policy.Find(text, delimiter);
  if (found_pos != StringPiece::npos) {
    found.reset(text.data() + found_pos, find_policy.Length(delimiter));
  }
  return found;
}
...
StringPiece FixedLength::Find(StringPiece text) const {
  ...
  if (text.size() <= length_)
    return StringPiece(text.end(), 0UL);

  text.advance(length_);
  return text;
}
...
void AppendToImpl(vector<string>* container, Splitter splitter) {
  ...
  for (size_t i = 0; i < vsp.size(); ++i) {
    (*container)[container_size++] = vsp[i].ToString();
  }
}
...
bool SplitStructuredLineInternal(StringPiece line,
  ...
  for (size_t i = 0; i < line.size(); ++i) {
    ...
    if (in_escape) {
      ...
      // We don't have any open expression, this is a valid separator.
      cols->back().subtract(line.size() - i);
