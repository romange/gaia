...
public:
  ...
  typedef std::char_traits<remove_const_type> traits_type;
...
public:
  ...
  template <class T = Iter, typename std::enable_if<
      std::is_same<T, char*>::value, int>::type = 0>
  /* implicit */ Range(std::string& str)
    : b_(&str.front()),
      e_(&str.front() + str.size()) {
  }
  ...
  template <class OtherIter, typename std::enable_if<
      (std::is_same<Iter, const char*>::value &&
       (std::is_same<OtherIter, const unsigned char*>::value ||
        std::is_same<OtherIter, unsigned char*>::value)), int>::type = 0>
  explicit Range(const OtherIter b, size_t len)
    : b_(reinterpret_cast<const char*>(b)),
      e_(reinterpret_cast<const char*>(b + len)) {
  }
  ...
  template <typename T = value_type, typename =
    typename std::enable_if<std::is_const<T>::value, int>::type>
  constexpr explicit Range(const std::vector<remove_const_type>& v)
  ...
  typename std::enable_if<std::is_same<T, const char*>::value, const unsigned char*>::type
    ubuf() const { return reinterpret_cast<const unsigned char*>(b_); }

  // Works only for Range<const char*> and Range<char*>
  std::string as_string() const { return std::string(b_, size()); }
  std::string ToString() const { return as_string(); }

  const_range_type cast_const() const {
  ...
  size_type find(const Iter s) const {
    return qfind(cast_const(), const_range_type(s));
  }

  // Works only for Range<(const) (unsigned) char*> which have Range(Iter) ctor
  size_type find(const Iter s, size_t pos) const {
    if (pos > size()) return std::string::npos;
    size_type ret = qfind(cast_const().subpiece(pos), const_range_type(s));
    return ret == npos ? ret : ret + pos;
  }

  size_type find(value_type c) const {
    return qfind(cast_const(), c);
  }

  size_type rfind(value_type c) const {
    return strings::rfind(cast_const(), c);
  }

  size_type find(value_type c, size_t pos) const {
    if (pos > size()) return std::string::npos;
    size_type ret = qfind(cast_const().subpiece(pos), c);
    return ret == npos ? ret : ret + pos;
  }

  size_type find_first_of(const_range_type needles) const {
    return qfind_first_of(cast_const(), needles);
  }

  size_type find_first_of(const_range_type needles, size_t pos) const {
    if (pos > size()) return std::string::npos;
    size_type ret = qfind_first_of(cast_const().subpiece(pos), needles);
    return ret == npos ? ret : ret + pos;
  }

  // Works only for Range<(const) (unsigned) char*> which have Range(Iter) ctor
  size_type find_first_not_of(const_range_type needles) const;
   ...
   */
  bool starts_with(const const_range_type& other) const {
    return size() >= other.size()
      && cast_const().subpiece(0, other.size()) == other;
  }
  ...
  template <class Comp>
  bool starts_with(const const_range_type& other, Comp&& eq) const {
    if (size() < other.size()) {
      return false;
    }
   ...
   */
  bool ends_with(const const_range_type& other) const {
    return size() >= other.size()
      && cast_const().subpiece(size() - other.size()) == other;
  }
  ...
  template <class Comp>
  bool ends_with(const const_range_type& other, Comp&& eq) const {
    if (size() < other.size()) {
      return false;
    }
   ...
   */
  bool remove_prefix(const const_range_type& prefix) {
    return starts_with(prefix) && (b_ += prefix.size(), true);
  }
   ...
   */
  bool remove_suffix(const const_range_type& suffix) {
    return ends_with(suffix) && (e_ -= suffix.size(), true);
  }
...
inline size_t qfind_first_byte_of(const StringPiece2 haystack,
  ...
  if ((needles.size() >= 4 && haystack.size() <= 10) ||
      ...
      needles.size() >= 32) {
    return qfind_first_byte_of_charset(haystack, needles);
  }
...
inline size_t find_first_not_of(const StringPiece2 haystack,
  ...
  return qfind_first_not_of_charset(haystack, needles);
}
...
template <class Iter> auto  Range<Iter>::find_first_not_of(const_range_type needles) const  ->
Range::size_type {
  return detail::find_first_not_of(cast_const(), needles);
}
