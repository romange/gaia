...
inline void EraseWhiteSpace(string* str) {
  auto end = std::remove_if(str->begin(), str->end(), ascii_isspace);
  str->erase(end, str->end());
}

inline void EraseAnyOf(const char* char_seq, string* str) {
  auto end = std::remove_if(str->begin(), str->end(),
      [char_seq](char c) {
        for (const char* i = char_seq; *i; ++i)  {
          if (*i == c)
            return true;
        }
        return false;
...
// ----------------------------------------------------------------------
uint32 TrimStringLeft(string* s, const StringPiece& remove);
...
//    of the string.  Returns the number of chars trimmed.
// ----------------------------------------------------------------------
uint32 TrimStringRight(string* s, const StringPiece& remove);
...
//    end of the string.
// ----------------------------------------------------------------------
inline uint32 TrimString(string* s, const StringPiece& remove) {
